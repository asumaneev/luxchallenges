...
template <typename container_t>
constexpr auto size(const container_t& c) noexcept {
    return c.size();
}

template <typename value_t, std::size_t array_size>
constexpr std::size_t size(const value_t (&array)[array_size]) noexcept {
    return array_size;
}
...
std::vector<value_of<container_t>> find_max_in_frame_impl(
    ...
    const iterator_t end = std::next(std::end(sequence), -frame_size + 1);

    std::vector<value_t> result;
    result.reserve(sequence_size);

    for (iterator_t begin_frame_it = std::begin(sequence);
         begin_frame_it != end;
         ++begin_frame_it) {
        const iterator_t end_frame_it = std::next(begin_frame_it, frame_size);
